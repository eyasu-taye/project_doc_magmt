  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      ...
      <ul class="nav navbar-nav navbar-right">
        ...
        <li><a id="btn_register" onclick="register_user()">Register_New_User</a></li>
        <li><a href="#Complated">Complated</a></li>
        <li><a href="#Inprogress">Inprogress</a></li>
        <li><a href="#Future">Future</a></li>
        <li><a href="#pspdfkit" onclick="login_user()">Login</a></li>
...
<div class="row"   id="Futurelist">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <!-- <button type="button" class="btn-info" data-dismiss="modal">ይቅር</button> -->
       <h4 class="modal-title" id="deletewarningheaddoc" style="float: left;"></h4><input name="x" type='button' class='btn btn-info' value='X' id='btn_cancle_upload' data-dismiss="modal" style="float: right;"> 
      </div>
      <!-- Form -->
        <form id="delete_data_doc" method='post' action="{{url('fileuploadtest')}}" enctype="multipart/form-data">
          {{csrf_field()}}
...
<!-- modla for registerig a project -->
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
       <h4 class="modal-title" id="conutselectedfiles" style="float: left;">Register New Project</h4>
...
<div id="editModal" class="modal fade" role="dialog" style = "top: 50px; z-index: 10040;">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
       <h4 class="modal-title" id="editmodalhead" style="float: left;">Register New Project</h4>
       <input type='button' class='btn btn-info' value='X' id='btn_cancle_upload' data-dismiss="modal" style="float: right;"> 
      </div>
      <!-- Form -->
        <form id="data" method='post' action="{{url('fileuploadtest')}}" enctype="multipart/form-data">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <!-- Form -->
        <form id="data" method='post' action="" enctype="multipart/form-data">
          {{csrf_field()}}  
           <label for="login_email_address">Email Address</label><br>         
          <input id="login_email_address" type="email" class="col-xs-12 btn btn-default" autofocus="true" style="border-radius: 5px red; text-align: right; display: block; float: left; row:1; left; margin-right: 5px;" placeholder="email_address">
           <label for="lgnpassword">Password</label><br>         
          <input id="lgnpassword" type="password" value="" class="col-xs-12 btn btn-default" autofocus="true" style="border-radius: 5px red; text-align: right; display: block; float: left; row:1; left; margin-right: 5px;" placeholder="password"><br>
        </form>
        <br>
        <small id="verifyuser" style="color: red;"></small>
      <div class="modal-body">
      </div>
      <div class="modal-footer">
         <input type='button' style="float: left;" class='btn btn-info' value='Cancel' id='btn_login_user' data-dismiss="modal"> <input type='button' onclick="login1()" class='btn btn-info' value='Send' id='btn_login_user'>
      </div>
    </div>
  </div>
</div>
...
for (var i = 0; i < btns.length; i++) {
  btns[i].addEventListener("click", function() {
    ...
    this.className += " active";
     ...
     function register_project(){
          // document.getElementById("mainbd").style.display = "none";
            ...
            alert("Opration not allowed please login to the system to register new projects");
      }
    ...
    function showdeletewarningfordoc(id){
              $("#deletewarningheaddoc").html("<h3>Deleting document</h3>");
              $("#delete_data_doc").html("<label>Are you sure you want to delete this document?</label>");
              $('#deletemodaldoc').modal({show: true});
               document.getElementById("btn_delete_doc").addEventListener('click', () => deletedoc(id));
        ...
        success: function (data) {
          ...
          for(j=1; j<=data.numofdoc; j++){
                          ...
                          switch (filetype) { 
                                      ...
                                      case "TIFF":
                                      case "RAW":
                                        $('#'+st).append("<div id= 'projectdoc' class='column' style='background-color:#101; border-radius:10px;'>"
                                          +"<h6><img id='imgfile' style='background-color:gray' src = '' class='fas fa-file-image file-icon' title='image'></i>"+data.filelocation[j]+"<span id='trash' onclick='showdeletewarningfordoc(this.id)' class='glyphicon glyphicon-trash' style='float:right;'> </span>"
                                          +"</h6><p><i>"+data.documet_description[j]+"</i><br></div>");
                                        document.getElementById('trash').id = data.docid[j];
                                        document.getElementById('projectdoc').id = "doc"+data.docid[j];
                                         document.getElementById('imgfile').id = "imgfile"+data.docid[j];
                                        var imgfile = document.getElementById("imgfile"+data.docid[j]);
                                            imgfile.src = data.absolutefilelocation[j];
                                            imgfile.width = 50;  // Set the width to 300 pixels
                                             imgfile.height = 30; // Set the height to 200 pixels
                                        break;
                                      case "pptx":
                                         $('#'+st).append("<div id= 'projectdoc' class='column' style='background-color:#101; border-radius:10px;'>"
    ...
    function showedit(id){
      ...
      function register_new_project(){
       ...
       function saveedit(){
            alert("Opration not allowed please login to the system to save your edits");                  
        } 
 ...
 function previewSelectedImage(thi, id) {
  ...
  document.getElementById("previewmultipleimg").innerHTML='';
            ...
            const filecaptiontext = document.getElementById('filecaptiontext');
                ...
                uploadimg(imageInput.files[0], id);
 ...
 var formData = new FormData();
  ...
  formData.append('filecaptiontext', $("#filecaptiontext").val());
    ...
    //                });
    alert("Opration is not allowed please login to the system to upload file");
            });
    ...
    //                });
    alert("Opration not allowed please login to the system to delete a project");
            });
//delete document
    function deletedoc(id) {
    //   var formDataDelete = new FormData();
    //       formDataDelete.append('deletedoc', id);
    //       formDataDelete.append('_token', '{{csrf_token()}}');
    // $.ajax({
    //    url:"{{url('deletedoc')}}",
    //     type: 'POST',
    ...
    //                       alert("Document deleted successfully " + "doc"+id);
    ...
    //                });
    alert("Opration not allowed please login to the system to delete a document");
            }
  ...
  // }
      ...
      function show_to_update_user(){
          ...
          function login1(){
                          ...
                          var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                          formData.append('_token', '{{csrf_token()}}');
                          formData.append('lgemail', $("#login_email_address").val());
                          formData.append('lgpassword', $("#lgnpassword").val());
                ...
                $.ajax({
                    ...
                    document.getElementById("verifyuser").innerHTML = "Your username or password is incorrect please try agnain";
                   }
                    },
                    error: function (xhr, status, error) {
                        alert(xhr.responseText);
